    ...
    public class NpcNode : AbstractNode {
        ...
        private void AddConversationPort(bool create, int index = -1) {
            ...
            if (create) {
                ...
                this.m_Lines.Add(new LineDataNpc {Line = ""});
            }
            ...
            Button removeButton = UIElementsFactory.Button("x", "conversation-item", "Remove line", new[] {"remove-button"}, () => { RemoveLine(this.m_Lines.FindIndex(data => data.PortGuidA == branchPort.viewDataKey)); });
        ...
        private void RemoveLine(int index) {
            ...
            List<SerializedEdge> edgesToRemove = Owner.EditorView.DlogObject.GraphData.Edges.Where(edge => edge.InputPort == this.m_Lines[index].PortGuidA || edge.OutputPort == this.m_Lines[index].PortGuidA).ToList();
            edgesToRemove.AddRange(Owner.EditorView.DlogObject.GraphData.Edges.Where(edge => edge.InputPort == this.m_Lines[index].PortGuidB || edge.OutputPort == this.m_Lines[index].PortGuidB));
            edgesToRemove.AddRange(Owner.EditorView.DlogObject.GraphData.Edges.Where(edge => edge.InputPort == this.m_Lines[index].PortGuidC || edge.OutputPort == this.m_Lines[index].PortGuidC));
            ...
            Owner.PortData.Remove(this.m_Lines[index].PortGuidC);
            ...
            Port portC = Owner.GuidPortDictionary[this.m_Lines[index].PortGuidC];
            Owner.GuidPortDictionary.Remove(this.m_Lines[index].PortGuidA);
            Owner.GuidPortDictionary.Remove(this.m_Lines[index].PortGuidB);
            Owner.GuidPortDictionary.Remove(this.m_Lines[index].PortGuidC);
            Ports.Remove(portA);
            Ports.Remove(portB);
            Ports.Remove(portC);
